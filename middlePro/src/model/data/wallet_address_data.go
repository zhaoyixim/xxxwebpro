...
// AddWalletAddress 创建钱包
func AddWalletAddressAndMechId(tokeninfo *mdb.WalletAddress) (*mdb.WalletAddress, error) {
	exist, err := GetWalletAddressByToken(tokeninfo.Token)
	if err != nil {
		return nil, err
	}
	if exist.ID > 0 {
...
// 禁用钱包
func ForBidWalletAddress(id int) error {
	err := dao.Mdb.Model(&mdb.WalletAddress{}).Where("id = ?", id).Update("status", 2).Error
	return err
}
