...
func CreateTransaction(req *request.CreateTransactionRequest) (*response.CreateTransactionResponse, error) {
	...
	orderCount := data.GetCountByUidAndItemId(req.Cuid, req.ItemId)
	order := &mdb.Orders{
	...
	findOrder, _ := data.GetOrderInfoByOrderId(order.OrderId)
	resp := &response.CreateTransactionResponse{
		...
		Token:          order.Token,
		CreatedAt:      findOrder.CreatedAt.Timestamp(),
	...
	// if findItemCuidData.IsLimitN == 0 { ////第一种

	// }
	// //第二模式
	// if findItemCuidData.IsLimitN == 1 {
	// }
	// //第三模式
	// if findItemCuidData.IsLimitN == 2 {
...
func OrderProcessing(
	...
	var returnItemList *mdb.GameItemList
	tx := dao.Mdb.Begin()
	exist, _ := data.GetOrderByBlockIdWithTransaction(tx, req.BlockTransactionId)
	if exist.ID > 0 {
		//tx.Rollback()
		fmt.Println("=OrderBlockAlreadyProcess=")
		fmt.Println(constant.OrderBlockAlreadyProcess)
		//return returnItemList
	}
	orderItemID := req.ItemId
	...
	GetOrderss, _ := data.GetOrderDatasByItemId(orderItemID)
	targetOrderLen := len(GetOrderss)

	data.UpdatePayNumberFlag(req.OrderId, targetOrderLen)

	GOItemDtlInfos, _ := data.GetGDtlById(orderItemID)
	//处理中奖信息
	relizeAmount := math.ChangeFloat64toInt(req.Amount)
	findItemCuidData, _ := data.GetLimitNumByGListId(order.ItemId, req.SCuid)
	totallPayedU := targetOrderLen * GOItemDtlInfos.PayUnit
	if findItemCuidData.IsLimitN == 0 { ////第一种
		if totallPayedU > GOItemDtlInfos.TotalU { //完成
			data.UpdateToExtraU(relizeAmount, req.SCuid, req.ItemId)
			data.UpdateOrderExtraFlag(order.ID)
		} else { //小于等于 未完成
			returnItemList, _ = data.DealGameReword(order, GOItemDtlInfos, targetOrderLen)
		}
	}
	//第二模式
	if findItemCuidData.IsLimitN == 1 { //小于等于   未完成
		returnItemList, _ = data.DealGameReword(order, GOItemDtlInfos, targetOrderLen)
	}

	//第三模式
	if findItemCuidData.IsLimitN == 2 && GOItemDtlInfos.PayCount > 0 {
		if targetOrderLen > GOItemDtlInfos.PayCount { //完成
			data.UpdateToExtraU(relizeAmount, req.SCuid, req.ItemId)
			data.UpdateOrderExtraFlag(order.ID)
		} else { //小于等于   未完成
			returnItemList, _ = data.DealGameReword(order, GOItemDtlInfos, targetOrderLen)
	...
	if returnItemList.ID == 0 {
		returnItemList, _ = data.GetGDtlById(orderItemID)
	}
