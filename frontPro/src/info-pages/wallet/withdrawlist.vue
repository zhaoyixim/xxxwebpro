...
const swipeActionOptionSelectEvent = async(
  ...
  optionIndex: number
	...
	let findId = withLists.value[0][index].id;
	const urlss="/g/delwithdraw/"+findId
	let backData = await that.$request.post({url:urlss})
	withLists.value[0].splice(index,1)
  // uni.showToast({
...
const withLists = ref([])
const getWithdrawLists= async() => {
	const urls ="/g/getwithdrawlist/"+memberinfo.id
	let backData = await that.$request.post({url:urls,data:pageInfo.value})
	withLists.value.push(backData)
}
getWithdrawLists()
onReachBottom(()=>{
...
<template>
  <CustomPage title="提现记录" :is-h5-demo-page="isDemoH5Page">
    <Container>
      <view class="swipe-action-container">
        <view v-if="withLists[0].length>0" class="swipe-action-item">
          <TnSwipeAction @select="swipeActionOptionSelectEvent">
            <TnSwipeActionItem
            v-for="(item,index) in withLists[0]"
            :key="index"
