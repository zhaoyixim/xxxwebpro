<script lang="ts" setup>
	...
	const listsurlunfinished = "/g/getunfinishedlist/"+QueryId
	const listsurlfinished = "/g/getfinishedlist/"+QueryId
					...
					<view v-for="(item,index) in tapDataUnfinishedLists" :key="index" class="shadow-item tn-shadow tn-type-primary_shadow">
						...
						<view @click="()=>toDtlPage(item)" class="secondTxtColor width100">							
							...
							<view class="font14 width100">
								...
								<view class="margintop10 font14 mainTxtColor">筹款：{{item.total_U}}U(总筹)/<text class="fourTxtColor">
								  {{item.charged_U>item.total_U?item.total_U:item.charged_U}}U(已筹)</text></view>	
