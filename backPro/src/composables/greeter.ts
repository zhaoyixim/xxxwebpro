...
import useMetaMask from "./tronlink";
...
function createContract(signer: JsonRpcSigner, chainId: number) {
  if (supportedChainIds.includes(chainId)) {
    ...
    const _contract = new ethers.Contract(contractAddress, contractData.abi, signer) as unknown as Greeter;
...
function connectContractAt(address: string) {
 ...
 // if (hasSetupWallet.value && signer.value && chainId.value) {
    ...
    contract = new ethers.Contract(address, contractData.abi, signer.value);
 // }
 console.log("contractData",contractData.abi)
 console.log("signer.value",JsonRpcSigner)
