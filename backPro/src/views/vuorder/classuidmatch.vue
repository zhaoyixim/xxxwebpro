<template>
  ...
  <classuidmatchAdd ref="classlistaddRef" @emitreloadTable="reloadTable"  :title="createSliderTitle" />

 <classuidmatchEdit ref="classlisteditRef" @emitreloadTable="reloadTable"  :title="createSliderTitle" />
...
<script lang="ts" setup>
  ...
  import { getItemMatchUidList ,updateClassClose , updateClassOpen} from "@/api/trade/lists";
  import { BasicTable, TableAction } from '@/components/Table';
  import { columns } from './classuidmatch';
  import { useMessage } from 'naive-ui';
  import { getMenuList ,operaShow} from '@/api/system/menu';
  import classuidmatchAdd from './classuidmatchadd.vue';
  import classuidmatchEdit from './classuidmatchedit.vue';
  ...
  const loadDataTable = async (res) => {
    ...
    let returnJoson = await getItemMatchUidList(_params);
    return returnJoson;
  };
  ...
  async function handleClose(record: Recordable) {
    ...
    //   return true;
    // }else{
      let backjson = await  updateClassClose({id:record.id,cuid:record.cuid})
  ...
  async function handleOpen(record: Recordable) {      
    if(record.cenable) {
      ...
      let backjson = await updateClassOpen({id:record.id,cuid:record.cuid})     
      if(backjson) record.cenable = !record.cenable
      message.info('操作成功');
      reloadTable();
    }    
