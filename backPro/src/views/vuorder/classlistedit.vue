<template>
  ...
  <script lang="ts">
    ...
    import { editItemData } from "@/api/vtemp/lists";
      ...
      setup(props,context) {
        ...
        const submitBtn = async() =>{
          let submitForm = {}
            ...
            let reb = await editItemData(submitForm)
            if(reb){
        ...
        function openDrawer(items:Object) {          
          itemData.data = items
          state.formParams.title = items.title
          state.formParams.cover = items.cover
          state.formParams.icon = items.icon
          state.formParams.path = items.path      
          state.formParams.url = items.url
          state.formParams.desp = items.desp
          state.formParams.is_show = items.is_show
          formDataReadOnly.data.forEach(it=>{           
